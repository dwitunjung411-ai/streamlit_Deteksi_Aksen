...
# Title
st.markdown('<h1 class="main-header">🎤 Klasifikasi Aksen Bahasa Indonesia</h1>', unsafe_allow_html=True)
...
def main():
    # Create tabs
    tab1, tab2, tab3 = st.tabs(["🎤 Prediksi Aksen", "📊 Analisis Audio", "ℹ️ Tentang Model"])
    ...
    with tab1:
        ...
        # Prediction button
        if st.button("🔍 Analisis Aksen", type="primary") and audio_file:
            with st.spinner("Memproses audio dan melakukan prediksi..."):
                try:
                    ...
                    if features is not None:
                        ...
                        with col_pred1:
                            st.metric("🎯 Aksen Terprediksi", predicted_accent)
                            st.metric("📊 Confidence", f"{probabilities[predicted_class_idx]*100:.1f}%")
        ...
        elif st.button("🔍 Analisis Aksen") and not audio_file:
    ...
    with tab3:
        ...
        1. **Embedding Network:**
           ...
           - Dense (128 units) → Embedding output
        ...
        st.markdown("### ⚙️ Parameter Model Saat Ini")
        params = {
            "n-way": n_way,
            "k-shot": k_shot,
            "q-query": q_query,
...
if __name__ == "__main__":
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 🔧 Informasi Teknis")
    ...
    ⚠️ <strong>Demo Simulasi</strong><br>
